...
export const MyReviews: React.FC<{ currentUser: FirebaseUser | UserProfile | { uid: string } | null }> = ({ currentUser: _currentUser }) => {
  ...
  return (
    <div className="space-y-6">
      <header>
        <h2 className="text-2xl font-sans font-bold text-zinc-900 mb-1">{t("dashboard.reviews.title", "Mes Évaluations")}</h2>
      ...
      <div className="bg-gradient-to-br from-indigo-50 to-blue-50 border border-indigo-100 rounded-2xl p-6 shadow-sm">
        ...
        {loading ? (
          <div className="h-20 animate-pulse bg-white/50 rounded-2xl" />
        ) : pendingReviews.length === 0 ? (
          <p className="text-sm text-indigo-800/60 font-medium p-4 bg-white/40 rounded-2xl text-center">
                ...
                >
                  <div className="flex items-center gap-4">
                    <img
                      ...
                      className="w-14 h-14 object-cover rounded-2xl border border-zinc-100"
                    />
                    <div>
                      <h4 className="font-bold text-sm text-zinc-900 line-clamp-1">{item.productName}</h4>
                  ...
                  <button
                    ...
                    className="w-full sm:w-auto px-5 py-2.5 bg-indigo-600 hover:bg-indigo-700 text-white font-bold text-[10px] rtl:text-[12px] uppercase tracking-widest rtl:tracking-normal rounded-2xl transition-colors shrink-0"
                  >
      ...
      <div className="bg-white rounded-2xl border border-zinc-200 overflow-hidden shadow-sm">
        <div className="p-5 border-b border-zinc-200 bg-transparent/50">
        ...
        {loading ? (
          ...
          <div className="p-10 text-center flex flex-col items-center">
            ...
            <p className="font-bold text-zinc-900 mb-1">{t("dashboard.reviews.no_published", "Aucun avis publié")}</p>
