...
export const BannerPreviewModal: React.FC<BannerPreviewModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm transition-opacity">
      <div className="bg-white rounded-2xl border border-zinc-150 shadow-2xl w-full max-w-5xl h-[90vh] flex flex-col overflow-hidden text-start">
        {/* Header */}
        ...
        <div className="flex-1 grid grid-cols-1 lg:grid-cols-4 overflow-hidden">
          ...
          <div className="lg:col-span-3 p-6 bg-zinc-100 flex flex-col items-center justify-center overflow-y-auto space-y-6">
            {/* Selector segment */}
            <div className="flex bg-white p-1 rounded-2xl border border-zinc-200 shadow-sm gap-1 select-none">
              <button
            ...
            <div className="w-full flex justify-center items-center flex-1">
                ...
                <div className="w-full max-w-3xl aspect-[2.4/1] rounded-2xl overflow-hidden bg-zinc-900 border border-zinc-300 relative shadow-lg">
                  ...
                  <div className="absolute inset-0 flex flex-col justify-end p-6 text-white text-start">
                    {tags.find((tg) => tg.id === previewBannerData.tag_id) && (
                      <span className="inline-flex items-center gap-0.5 px-2 py-0.5 rounded bg-white/15 tracking-widest uppercase font-sans font-bold text-xs w-fit mb-1.5">
                    ...
                    <button
                      ...
                      className="rounded-2xl py-1.5 px-4 text-xs uppercase tracking-widest font-sans font-bold shrink-0 w-fit pointer-events-none mt-1 shadow-md"
                    >
              ...
              {previewDeviceMode === "tablet" && (
                <div className="w-[500px] aspect-[1.6/1] rounded-2xl overflow-hidden bg-zinc-900 border border-zinc-300 relative shadow-lg">
                  ...
                  <div className="absolute inset-0 flex flex-col justify-end p-5 text-white text-start">
                    ...
                    {previewBannerData.subtitle && (
                      <p className="text-xs font-semibold leading-normal mb-2 opacity-90" style={{ color: previewBannerData.subtitle_color }}>
                        {previewBannerData.subtitle}
                      </p>
                    )}
                    <button
              ...
              {previewDeviceMode === "mobile" && (
                <div className="w-64 aspect-[4/5] rounded-2xl overflow-hidden bg-zinc-900 border border-zinc-300 relative shadow-lg">
                  {previewBannerData.mobile_image ? (
                    ...
                    <div className="w-full h-full relative">
                      <img
                        ...
                        referrerPolicy="no-referrer"
                      />
                      <div className="absolute top-2 start-2 px-1.5 py-0.5 bg-orange-600/95 rounded text-xs font-sans font-bold text-white uppercase tracking-wider leading-none select-none">
                        {t("Desktop Fallback")}
                      </div>
                    </div>
                  )}
                  ...
                  <div className="absolute inset-0 flex flex-col justify-end p-4 text-white text-start">
                    {tags.find((tg) => tg.id === previewBannerData.tag_id) && (
                      <span className="inline-block tracking-widest uppercase font-sans font-bold text-xs text-zinc-300 mb-0.5">
                        {tags.find((tg) => tg.id === previewBannerData.tag_id)?.name}
                      </span>
                    )}
                    ...
                    {previewBannerData.subtitle && (
                      <p className="text-xs font-semibold leading-tight mb-1 opacity-90 truncate" style={{ color: previewBannerData.subtitle_color }}>
                        {previewBannerData.subtitle}
                      </p>
                    )}
                    <button
                      style={{ backgroundColor: previewBannerData.btn_bg_color || "#FFFFFF", color: previewBannerData.btn_text_color || "#18181B" }}
                      className="rounded py-1 px-2.5 text-xs uppercase tracking-widest font-sans font-bold shrink-0 w-fit pointer-events-none block shadow-sm mt-1"
                    >
                      {previewBannerData.button_text}
                    </button>
                  </div>
          ...
          <div className="p-6 border-t lg:border-t-0 lg:border-l border-zinc-150 space-y-5 overflow-y-auto shrink-0">
            <div>
              <h4 className="text-xs font-sans font-bold uppercase tracking-widest text-zinc-400">{t("Analyse de la Fiche")}</h4>
            ...
            <div className="pt-4 border-t border-zinc-150 space-y-3">
              <h5 className="text-xs font-black uppercase text-zinc-900 tracking-wider">{t("📊 Statistiques réelles")}</h5>
              
              <div className="grid grid-cols-2 gap-2 text-center">
                <div className="p-2.5 bg-zinc-50 border border-zinc-150 rounded-2xl">
                  <span className="block text-xs font-bold text-zinc-400 uppercase">{t("Vues")}</span>
                  <span className="text-sm font-black text-zinc-800 font-mono">{previewBannerData.views || 0}</span>
                </div>
                <div className="p-2.5 bg-zinc-50 border border-zinc-150 rounded-2xl">
                  <span className="block text-xs font-bold text-zinc-400 uppercase">{t("Clics")}</span>
                  <span className="text-sm font-black text-zinc-800 font-mono">{previewBannerData.clicks || 0}</span>
                </div>
              </div>

              <div className="p-3 bg-orange-50 border border-orange-100 rounded-2xl flex items-center justify-between text-orange-800 font-bold uppercase text-xs">
            ...
            <div className="pt-4 border-t border-zinc-150 space-y-3.5 text-xs text-zinc-700">
              <h5 className="text-xs font-black uppercase text-zinc-900 tracking-wider">{t("🎯 Paramètres de Ciblage")}</h5>
              
              <div className="space-y-2">
                <div className="flex justify-between">
                  <span className="text-zinc-400 uppercase text-xs font-bold">{t("Zone d'affichage")}</span>
                ...
                <div className="flex justify-between">
                  <span className="text-zinc-400 uppercase text-xs font-bold">{t("Groupe A/B Test")}</span>
                  <span className="font-extrabold text-zinc-900 text-xs uppercase">
                ...
                <div className="flex justify-between">
                  <span className="text-zinc-400 uppercase text-xs font-bold">{t("Cible Visiteur")}</span>
                  <span className="font-extrabold text-zinc-900 text-xs uppercase">
                ...
                <div className="space-y-1">
                  <span className="text-zinc-400 uppercase text-xs font-bold block">{t("Wilayas Ciblées")}</span>
                  <div className="flex flex-wrap gap-1 max-h-[80px] overflow-y-auto">
                    {previewBannerData.target_regions && previewBannerData.target_regions.length > 0 ? (
                      previewBannerData.target_regions.map((w: string) => (
                        <span key={w} className="px-1.5 py-0.5 rounded bg-zinc-100 text-zinc-800 text-xs font-bold uppercase">
                          {w}
                        </span>
                      ))
                    ) : (
                ...
                <div className="space-y-1 pt-1.5 border-t border-dashed border-zinc-200">
                  <span className="text-zinc-400 uppercase text-xs font-bold block">{t("Planification horaire")}</span>
                  {previewBannerData.start_date || previewBannerData.end_date ? (
                    <div className="text-xs font-bold text-zinc-800 font-mono space-y-0.5 leading-tight">
                    ...
                    <span className="text-xs text-emerald-600 uppercase font-black">{t("Permanent (Illimité)")}</span>
                  )}
                </div>
              </div>
            </div>
            ...
            <div className="pt-4 border-t border-zinc-150 space-y-2">
              <h5 className="text-xs font-black uppercase text-zinc-900 tracking-wider">{t("🏷️ Tag de redirection")}</h5>
              {tags.find((tg) => tg.id === previewBannerData.tag_id) ? (
                <div className="p-3 bg-zinc-50 border border-zinc-200 rounded-2xl flex items-center gap-2">
                  <Tag className="w-4 h-4 text-orange-600 shrink-0" />
                  <div>
                    <p className="text-xs font-extrabold text-zinc-900 uppercase">
                      {tags.find((tg) => tg.id === previewBannerData.tag_id)?.name}
                    </p>
                    <p className="text-xs font-mono text-zinc-400 leading-none mt-0.5">
                      slug: /{tags.find((tg) => tg.id === previewBannerData.tag_id)?.slug}
                    </p>
                  </div>
                </div>
              ) : (
                <div className="p-3 bg-red-50 border border-red-100 rounded-2xl text-red-600 font-bold uppercase text-xs">
