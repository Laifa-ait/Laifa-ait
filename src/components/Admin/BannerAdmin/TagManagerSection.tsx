...
export const TagManagerSection: React.FC<TagManagerSectionProps> = ({
  ...
  return (
    <div id="tab-content-tags" className="grid grid-cols-1 lg:grid-cols-3 gap-8 text-start animate-fade-in">
      {/* Creating tag form */}
      <div id="tag-creator-card" className="bg-white p-6 rounded-2xl border border-zinc-100 shadow-sm h-fit space-y-5">
        <div>
          <h3 className="text-base font-extrabold text-zinc-950 uppercase tracking-wide">
            {t("Créer un Nouveau Tag")}
          </h3>
          <p className="text-xs font-bold text-zinc-400 uppercase tracking-wider mt-0.5">
        ...
        <form id="tag-create-form" onSubmit={handleCreateTag} className="space-y-4">
          ...
          <div className="space-y-1.5">
            ...
            <input
              ...
              className="w-full h-11 px-4 rounded-2xl border border-zinc-200 text-sm bg-zinc-50 focus:outline-none font-mono"
            />
          </div>
          ...
          <button
            id="btn-submit-tag"
            type="submit"
            className="w-full h-11 bg-orange-600 text-white rounded-2xl font-sans font-bold text-xs uppercase tracking-widest hover:bg-orange-500 transition-colors cursor-pointer flex items-center justify-center gap-2 border-none"
          >
      ...
      {/* Tags list */}
      <div id="tags-list-card" className="lg:col-span-2 bg-white p-6 rounded-2xl border border-zinc-100 shadow-sm space-y-4">
        <h3 className="text-base font-extrabold text-zinc-950 uppercase tracking-wide">
          {t("Tags Existants (")}
          {tags.length})
        </h3>
            ...
            {tags.map((tag) => (
              ...
              >
                <div className="space-y-0.5 min-w-0">
                  ...
                  <div className="text-xs font-mono text-zinc-400 font-bold tracking-wider truncate">
                    {t("slug:")}
                    {tag.slug}
                  </div>
                </div>
                ...
                <button
                  ...
                  className="p-2 text-zinc-400 hover:text-red-500 hover:bg-red-50 rounded-2xl transition-all shrink-0 cursor-pointer border-none"
