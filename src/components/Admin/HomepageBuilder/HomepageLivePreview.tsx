...
export const HomepageLivePreview: React.FC<HomepageLivePreviewProps> = ({
  ...
  return (
    <div className="bg-zinc-900 text-white rounded-2xl p-6 shadow-2xl border border-zinc-800 space-y-6" id="homepage-live-preview">
      {/* Top Controls Bar */}
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-4 border-b border-zinc-800">
        ...
        <div className="flex items-center gap-3">
          <div className="flex bg-zinc-800 rounded-2xl p-1 border border-zinc-700">
            <button
              ...
              className={`p-2 rounded-lg text-xs font-bold transition-all cursor-pointer ${
                device === "desktop" ? "bg-amber-500 text-white" : "text-zinc-400 hover:text-white"
              }`}
              title="Bureau"
            >
            ...
            <button
              ...
              className={`p-2 rounded-lg text-xs font-bold transition-all cursor-pointer ${
                device === "tablet" ? "bg-amber-500 text-white" : "text-zinc-400 hover:text-white"
              }`}
              title="Tablette"
            >
            ...
            <button
              ...
              className={`p-2 rounded-lg text-xs font-bold transition-all cursor-pointer ${
                device === "mobile" ? "bg-amber-500 text-white" : "text-zinc-400 hover:text-white"
              }`}
              title="Mobile"
            >
          ...
          <div className="flex items-center gap-2 bg-zinc-800 px-3 py-1.5 rounded-2xl border border-zinc-700 text-xs">
            ...
            >
              <option value="Toutes" className="bg-zinc-800">Toutes les wilayas</option>
              <option value="16 - Alger" className="bg-zinc-800">16 - Alger</option>
              <option value="31 - Oran" className="bg-zinc-800">31 - Oran</option>
              <option value="25 - Constantine" className="bg-zinc-800">25 - Constantine</option>
              <option value="19 - Sétif" className="bg-zinc-800">19 - Sétif</option>
            </select>
          </div>
          ...
          {onClose && (
            <button
              type="button"
              onClick={onClose}
              className="px-3 py-2 rounded-2xl bg-zinc-800 hover:bg-zinc-700 text-zinc-300 text-xs font-bold transition-colors cursor-pointer"
            >
              {t("Fermer")}
            </button>
          )}
      ...
      <div className="flex justify-center p-4 bg-zinc-950/60 rounded-2xl overflow-x-auto">
        <div className={`${getDeviceWidthClass()} bg-[#FAF8F5] text-zinc-900 rounded-2xl shadow-2xl transition-all duration-300 border-4 border-zinc-800 overflow-hidden flex flex-col`}>
          {/* Header */}
          <div className="bg-zinc-200 px-4 py-2 flex items-center justify-between border-b border-zinc-300">
            <div className="flex items-center gap-1.5">
              <span className="w-2.5 h-2.5 rounded-full bg-rose-400" />
              <span className="w-2.5 h-2.5 rounded-full bg-amber-400" />
              <span className="w-2.5 h-2.5 rounded-full bg-emerald-400" />
            </div>
            <div className="text-[10px] font-mono text-zinc-600 bg-white/70 px-4 py-0.5 rounded-lg border border-zinc-300/60 truncate max-w-[200px] sm:max-w-xs">
          ...
          <div className="p-4 sm:p-6 space-y-6 max-h-[700px] overflow-y-auto">
            <div className="rounded-2xl bg-gradient-to-r from-teal-900 via-zinc-900 to-amber-950 text-white p-6 shadow-md relative overflow-hidden">
              <div className="relative z-10 max-w-md space-y-2">
                <span className="px-2.5 py-1 rounded-full text-[9px] font-extrabold uppercase tracking-widest bg-amber-400 text-zinc-950 inline-block">
                  Édition 2026
                </span>
