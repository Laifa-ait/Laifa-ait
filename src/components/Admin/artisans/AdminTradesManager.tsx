  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center bg-white p-4 rounded-2xl border border-zinc-200 shadow-xs">
        <div>
          <h2 className="text-lg font-black text-zinc-900">Catégories de Métiers</h2>
        ...
        <button
          ...
          className="px-4 py-2 rounded-2xl bg-zinc-900 hover:bg-zinc-800 text-white font-bold text-xs flex items-center gap-2 cursor-pointer transition-colors"
        >
      ...
      {(isCreating || editingTrade) && (
        <form onSubmit={handleSave} className="bg-zinc-50 p-6 rounded-2xl border border-zinc-200 space-y-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              <label className="block text-xs font-bold text-zinc-700 mb-1">Nom du Métier *</label>
              <input
                type="text"
                required
                ...
                className="w-full px-3 py-2 rounded-2xl border border-zinc-200 text-xs font-medium bg-white"
                placeholder="ex: Électricien"
              />
            </div>

            <div>
              <label className="block text-xs font-bold text-zinc-700 mb-1">Slug URL *</label>
              <input
                type="text"
                required
          ...
          <div className="flex justify-end gap-2 pt-2">
            <button
              ...
              className="px-4 py-2 rounded-2xl bg-zinc-200 text-zinc-700 font-bold text-xs cursor-pointer"
            >
              Annuler
            </button>
            <button
              type="submit"
              disabled={submitting}
              className="px-4 py-2 rounded-2xl bg-emerald-600 hover:bg-emerald-700 text-white font-bold text-xs cursor-pointer flex items-center gap-1.5"
            >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        {trades.map((trade) => (
          <div key={trade.id} className="bg-white p-5 rounded-2xl border border-zinc-200 shadow-xs space-y-3">
            <div className="flex justify-between items-start">
              <div className="flex items-center gap-2">
                <span className="p-2 rounded-2xl bg-amber-50 text-amber-700 font-bold">
                  <Tag className="w-4 h-4" />
                </span>
                <div>
                  <h4 className="font-bold text-zinc-900 text-sm">{trade.name}</h4>
                  <p className="text-[11px] text-zinc-400 font-mono">{trade.slug}</p>
                </div>
              </div>
            ...
            {trade.specialties && trade.specialties.length > 0 && (
              <div className="flex flex-wrap gap-1 pt-1">
                {trade.specialties.slice(0, 3).map((spec, i) => (
                  <span key={i} className="px-2 py-0.5 rounded-lg bg-zinc-100 text-zinc-600 text-[10px] font-bold">
                    {spec}
                  </span>
                ))}
              </div>
