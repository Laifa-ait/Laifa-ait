        ...
        <div className="grid grid-cols-2 gap-3">
          <button
            ...
            className={`p-3 rounded-2xl border-2 text-left transition cursor-pointer flex items-center justify-between ${
              !isPriceNegotiable ? 'border-[#1E3A8A] bg-blue-50/70 shadow-2xs' : 'border-slate-200 bg-white hover:border-slate-300'
            }`}
          >
            <div>
              <span className="text-xs font-bold text-[#1E3A8A] block">Prix Ferme / Non négociable</span>
              <span className="text-[10px] text-slate-500 block">Le montant est définitif</span>
            </div>
            {!isPriceNegotiable && <Check className="w-4 h-4 text-[#1E3A8A] stroke-[3]" />}
          </button>
          ...
          <button
            ...
            className={`p-3 rounded-2xl border-2 text-left transition cursor-pointer flex items-center justify-between ${
              isPriceNegotiable ? 'border-amber-600 bg-amber-50 shadow-2xs' : 'border-slate-200 bg-white hover:border-slate-300'
            }`}
          >
            <div>
      ...
      {listingType === 'rent_long' && (
        <div className="p-3.5 bg-slate-50 rounded-2xl border border-slate-200 space-y-3.5">
          ...
          <div className="space-y-2">
            <label className="text-xs font-bold text-[#1E3A8A] block">Avance exigée à la signature :</label>
                ...
                return (
                  <button
                    ...
                    className={`p-2 rounded-xl border text-center transition cursor-pointer flex flex-col items-center justify-center ${
                      isSel ? 'bg-[#1E3A8A] text-white border-[#1E3A8A] font-bold' : 'bg-white text-slate-700 border-slate-200 hover:border-slate-300'
                    }`}
                  >
                    <span className="text-xs font-extrabold">{opt.label}</span>
                    <span className={`text-[9px] ${isSel ? 'text-amber-300 font-semibold' : 'text-slate-400'}`}>{opt.badge}</span>
                  </button>
                );
              })}
            </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 pt-1">
            <div className="space-y-1">
              <label className="text-xs font-bold text-[#1E3A8A] block">Caution de garantie (Mois de loyer) :</label>
              <div className="flex items-center gap-2">
                <input
                  ...
                  className="w-24 px-3 py-1.5 bg-white border border-slate-200 rounded-xl text-xs font-bold text-[#1E3A8A] focus:outline-none focus:border-[#1E3A8A]"
                />
