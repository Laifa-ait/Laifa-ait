  ...
  return (
    <div className="flex flex-wrap items-center justify-between gap-2 text-xs">
      <div className="flex items-center gap-1.5 font-bold text-[#1E3A8A]">
        <Crosshair className="w-4 h-4 text-[#1E3A8A]" />
        ...
        <div className="inline-flex p-0.5 rounded-xl bg-slate-200 border border-slate-300">
          <button
            ...
            className={`px-2.5 py-1 rounded-lg font-bold text-[11px] flex items-center gap-1 transition cursor-pointer ${
              layerType === 'satellite' ? 'bg-[#1E3A8A] text-white shadow-xs' : 'text-slate-700 hover:text-slate-900'
            }`}
          >
          ...
          <button
            ...
            className={`px-2.5 py-1 rounded-lg font-bold text-[11px] flex items-center gap-1 transition cursor-pointer ${
              layerType !== 'satellite' ? 'bg-white text-[#1E3A8A] shadow-xs' : 'text-slate-700 hover:text-slate-900'
            }`}
          >
            <span>Plan</span>
          </button>
...
> = ({
  ...
  return (
    <>
      ...
      >
        <span className="text-[10px] font-black uppercase tracking-wider text-slate-900 bg-[#F59E0B] px-2 py-0.5 rounded-full shadow-md border border-amber-400 mb-0.5 whitespace-nowrap">
          {communeName || wilayaName ? `Logement (${communeName || wilayaName})` : 'Emplacement du bien'}
        </span>
        <div className="p-2 bg-[#1E3A8A] text-white rounded-full shadow-xl border-2 border-white">
          <MapPin className="w-5 h-5 text-[#F59E0B] drop-shadow-xs" />
        </div>
        <div className="w-2 h-2 bg-[#F59E0B] rotate-45 -mt-1 shadow-2xs" />
      </div>
      ...
      <div className="absolute bottom-3 right-3 z-30 flex items-end gap-2">
        ...
        {onNudge && (
          <div className="flex flex-col items-center p-1 bg-white/95 backdrop-blur-md rounded-xl border border-slate-300 shadow-md">
            <button
              type="button"
              onClick={(e) => {
                e.stopPropagation();
              ...
              className="w-7 h-7 flex items-center justify-center text-slate-700 hover:text-[#1E3A8A] hover:bg-slate-100 rounded-lg transition active:scale-95 cursor-pointer"
            >
