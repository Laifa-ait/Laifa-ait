    ...
    <div className="pt-4 border-t border-zinc-100 space-y-4">
      ...
      {setLegalPaperFilter && (
        <div className="space-y-2.5">
          <div className="flex items-center justify-between">
            ...
            <span className="text-[10px] text-zinc-400 font-medium">Cadastre & Notariat</span>
          </div>

          {/* Dedicated Checkbox Filters for "Acte notarié disponible" & "Livret foncier disponible" */}
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 p-3 bg-zinc-50/80 rounded-2xl border border-zinc-200/80">
            <label className="flex items-center gap-2.5 p-2 rounded-2xl bg-white border border-zinc-200 text-xs font-semibold text-zinc-800 hover:border-emerald-400 cursor-pointer transition select-none">
              <input
                type="checkbox"
                checked={Boolean(hasActeNotarie)}
                onChange={(e) => onToggleActeNotarie?.(e.target.checked)}
                className="w-4 h-4 rounded text-emerald-600 focus:ring-emerald-500 border-zinc-300 accent-emerald-600 cursor-pointer"
              />
              <span className="flex-1">Acte notarié disponible</span>
              <span className="text-[10px] font-bold text-emerald-700 bg-emerald-50 border border-emerald-200 px-1.5 py-0.5 rounded-lg">
                Notarié
              </span>
            </label>

            <label className="flex items-center gap-2.5 p-2 rounded-2xl bg-white border border-zinc-200 text-xs font-semibold text-zinc-800 hover:border-[#1a3831] cursor-pointer transition select-none">
              <input
                type="checkbox"
                checked={Boolean(hasLivretFoncier)}
                onChange={(e) => onToggleLivretFoncier?.(e.target.checked)}
                className="w-4 h-4 rounded text-[#1a3831] focus:ring-[#1a3831] border-zinc-300 accent-[#1a3831] cursor-pointer"
              />
              <span className="flex-1">Livret foncier disponible</span>
              <span className="text-[10px] font-bold text-[#1a3831] bg-[#f4ecd8] border border-[#e8e2d4] px-1.5 py-0.5 rounded-lg">
                Cadastre
              </span>
            </label>
          </div>
          ...
          <div className="flex flex-wrap gap-1.5">
            <button
              ...
              className={`px-3 py-1.5 rounded-2xl text-xs font-semibold transition-all cursor-pointer min-h-[36px] ${
                legalPaperFilter === 'all'
            ...
            {Object.values(LEGAL_PAPERS_CONFIG).map((paper) => {
              ...
              return (
                <button
                  ...
                  className={`px-3 py-1.5 rounded-2xl text-xs font-semibold transition-all cursor-pointer min-h-[36px] flex items-center gap-1.5 ${
                    isSelected
          ...
          {AMENITIES_OPTIONS.map((item) => {
            ...
            return (
              <button
                ...
                className={`px-2.5 py-1 rounded-2xl text-xs transition-colors cursor-pointer min-h-[32px] ${
                  isSelected
      ...
      <div className="flex justify-end pt-2">
        <button
          type="button"
          onClick={onReset}
          className="text-xs text-zinc-500 hover:text-zinc-800 font-semibold underline cursor-pointer"
        >
