...
export const ImageGalleryLightbox: React.FC<ImageGalleryLightboxProps> = ({
  ...
  return (
    <div
      role="dialog"
      aria-modal="true"
      aria-label={`Galerie photos de l'annonce : ${title}`}
      ...
      <div className="flex items-center justify-between text-white z-10">
        ...
        <div className="flex items-center gap-3">
          ...
          <button
            ref={closeBtnRef}
            type="button"
            aria-label="Fermer la galerie photo (Échap)"
            onClick={onClose}
            className="p-2.5 rounded-full bg-white/10 hover:bg-white/20 text-white transition-colors cursor-pointer min-w-[44px] min-h-[44px] flex items-center justify-center focus:ring-2 focus:ring-emerald-400"
            title="Fermer (Échap)"
          >
            <X className="w-6 h-6" />
          </button>
      ...
      <div className="relative flex-1 flex items-center justify-center my-4 overflow-hidden">
        {images.length > 1 && (
          <button
            ...
            onClick={handlePrev}
            className="absolute left-2 sm:left-4 z-20 p-3 rounded-full bg-black/50 hover:bg-black/80 text-white transition-all cursor-pointer border border-white/20 hover:scale-110 min-w-[48px] min-h-[48px] flex items-center justify-center focus:ring-2 focus:ring-emerald-400"
        ...
        {images.length > 1 && (
          <button
            ...
            onClick={handleNext}
            className="absolute right-2 sm:right-4 z-20 p-3 rounded-full bg-black/50 hover:bg-black/80 text-white transition-all cursor-pointer border border-white/20 hover:scale-110 min-w-[48px] min-h-[48px] flex items-center justify-center focus:ring-2 focus:ring-emerald-400"
        ...
        <div className="flex items-center justify-center gap-2 overflow-x-auto py-2 scrollbar-none z-10 max-w-4xl mx-auto w-full">
          {images.map((img, idx) => (
            <button
              ...
              className={`relative w-16 h-12 rounded-lg overflow-hidden shrink-0 border-2 transition-all cursor-pointer min-w-[44px] min-h-[44px] ${
                currentIndex === idx
