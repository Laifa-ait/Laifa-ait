...
export const DisputeChat: React.FC<{ disputeId: string, onClose?: () => void }> = ({ disputeId, onClose }) => {
  ...
  return (
    <div className="flex flex-col h-[550px] bg-white rounded-2xl shadow-xl overflow-hidden border border-zinc-200">
      <div className="flex items-center justify-between p-4 bg-zinc-50 border-b border-zinc-200 shrink-0">
        <div>
          <h3 className="font-bold text-zinc-800">{t('Litige sur commande')} #{dispute?.orderId?.substring(0, 8)}</h3>
          ...
          messages.map((msg, idx) => {
            ...
            return (
              <div key={msg.id || idx} className={`flex ${isMe ? 'justify-end' : 'justify-start'} mb-4`}>
                {!isMe && (
                  <div className={`w-8 h-8 rounded-full flex items-center justify-center mr-2 shrink-0 ${isAdmin ? 'bg-zinc-800 text-white' : 'bg-orange-100 text-orange-600'}`}>
                    {isAdmin ? <ShieldCheck className="w-4 h-4" /> : <User className="w-4 h-4" />}
                  </div>
                )}
                <div className={`max-w-[75%] rounded-2xl p-3 ${isMe ? 'bg-orange-600 text-white rounded-br-none' : isAdmin ? 'bg-zinc-800 text-white rounded-bl-none' : 'bg-white border border-zinc-200 text-zinc-800 rounded-bl-none shadow-sm'}`}>
                  {isAdmin && !isMe && <div className="text-[10px] uppercase font-bold text-zinc-300 mb-1">Support Olmart</div>}
                  ...
                  {msg.fileUrl && (
                    <div className="mt-2.5 border-t border-zinc-100/50 pt-2 space-y-2">
                      {msg.fileType?.startsWith('image/') ? (
                        <div onClick={() => setLightboxImg(msg.fileUrl || null)} className="max-w-[180px] rounded-lg overflow-hidden border border-zinc-200 cursor-pointer hover:opacity-90 transition-opacity bg-zinc-50">
                        ...
                        <a href={msg.fileUrl} target="_blank" rel="noopener noreferrer" className={`inline-flex items-center gap-2 px-3 py-2 rounded-2xl text-xs font-semibold border ${isMe ? 'bg-orange-700 border-orange-550 text-white hover:bg-orange-800' : 'bg-zinc-100 border-zinc-200 text-zinc-800 hover:bg-zinc-200'} transition-all`}>
      ...
      <div className="p-4 bg-white border-t border-zinc-200 flex items-center gap-2 shrink-0">
        <button onClick={() => fileInputRef.current?.click()} disabled={dispute?.status === 'resolved' || sending || uploading} className="p-3 hover:bg-zinc-100 rounded-full transition-colors text-zinc-500 disabled:opacity-50 relative shrink-0 cursor-pointer" title={t('Ajouter une preuve (Max 1 Mo)')}>
        ...
        <input type="text" value={newMessage} onChange={e => setNewMessage(e.target.value)} onKeyDown={e => e.key === 'Enter' && handleSend()} placeholder={t('write_message', 'Écrire un message...')} className="flex-1 bg-zinc-100 border-none rounded-full px-4 py-3 text-sm focus:ring-2 focus:ring-orange-500 outline-none" disabled={dispute?.status === 'resolved' || sending} />
