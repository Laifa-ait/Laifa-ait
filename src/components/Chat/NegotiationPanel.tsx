...
export const NegotiationPanel: React.FC<NegotiationPanelProps> = ({
  ...
  return (
    <div className="bg-[#fefbf3] border border-[#e8dcc4] rounded-2xl p-4 my-3 text-zinc-800 shadow-xs">
      {/* Header */}
      <div className="flex items-center justify-between pb-3 border-b border-[#e8e2d4]">
        <div className="flex items-center gap-2">
          <div className="p-2 bg-[#f4ecd8] text-[#1e3835] rounded-2xl border border-[#e8e2d4]">
        ...
        <div>
          ...
          {negotiation.status === 'CANCELLED' && (
            <span className="inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-bold bg-zinc-100 text-zinc-700 border border-zinc-300">
      ...
      <div className="grid grid-cols-2 gap-3 my-3">
        {negotiation.initialPriceDZD > 0 && (
          <div className="bg-white rounded-2xl p-2.5 border border-[#e8e2d4]">
            <span className="text-[11px] text-zinc-400 block font-medium">Prix initial</span>
        ...
        <div className={`bg-white rounded-2xl p-2.5 border ${negotiation.initialPriceDZD > 0 ? 'border-[#e8e2d4]' : 'col-span-2 border-[#e8e2d4]'}`}>
      ...
      {/* Terms or Notes */}
      {negotiation.terms && (
        <div className="text-xs text-zinc-700 bg-white p-2.5 rounded-2xl border border-[#e8e2d4] mb-3">
          <span className="text-[10px] uppercase font-bold text-zinc-400 block mb-0.5">Note de l'émetteur :</span>
          {negotiation.terms}
        </div>
      )}
              ...
              {!showCounterInput ? (
                <div className="flex items-center gap-2">
                  <button
                    ...
                    className="flex-1 py-2 px-3 bg-[#1e3835] hover:bg-[#152725] disabled:opacity-50 text-white rounded-2xl text-xs font-bold transition flex items-center justify-center gap-1.5 shadow-xs cursor-pointer min-h-[38px]"
                  >
                  ...
                  <button
                    ...
                    disabled={loadingAction !== null}
                    className="flex-1 py-2 px-3 bg-[#f4ecd8] hover:bg-[#ebdcb8] disabled:opacity-50 text-[#1e3835] border border-[#e8e2d4] rounded-2xl text-xs font-bold transition flex items-center justify-center gap-1.5 cursor-pointer min-h-[38px]"
                  >
                  ...
                  <button
                    ...
                    disabled={loadingAction !== null}
                    className="py-2 px-3 bg-rose-50 hover:bg-rose-100 text-rose-700 border border-rose-200 disabled:opacity-50 rounded-2xl text-xs font-bold transition flex items-center justify-center gap-1 cursor-pointer min-h-[38px]"
                  >
                ...
                <form onSubmit={handleCounterSubmit} className="space-y-2">
                  <div className="flex items-center gap-2">
                    <input
                      ...
                      disabled={loadingAction !== null}
                      className="flex-1 bg-white border border-[#e8e2d4] rounded-2xl px-3 py-2 text-xs text-zinc-900 placeholder-zinc-400 focus:outline-hidden focus:border-[#1e3835]"
                    />
                    <button
                      type="submit"
                      disabled={loadingAction !== null || !counterAmount}
                      className="py-2 px-3 bg-[#1e3835] hover:bg-[#152725] disabled:opacity-50 text-white rounded-2xl text-xs font-bold flex items-center gap-1 cursor-pointer min-h-[36px]"
                    >
                    ...
                    <button
                      ...
                      className="py-2 px-2.5 bg-[#f4ecd8] text-[#1e3835] hover:bg-[#ebdcb8] rounded-2xl text-xs font-bold cursor-pointer min-h-[36px]"
                    >
                      Annuler
                    </button>
                  </div>
          ...
          {isProposer && (
            <div className="flex items-center justify-between">
              ...
              <button
                ...
                disabled={loadingAction !== null}
                className="py-1.5 px-3 bg-white hover:bg-zinc-50 disabled:opacity-50 text-zinc-700 border border-[#e8e2d4] rounded-2xl text-xs font-bold transition flex items-center gap-1.5 cursor-pointer min-h-[32px]"
              >
