...
export const ComparatorDrawer: React.FC = () => {
  ...
  return (
    <AnimatePresence>
      ...
      >
        <div className="bg-white/95 backdrop-blur-xl border border-amber-200/90 text-slate-900 shadow-[0_16px_40px_rgba(217,119,6,0.12)] rounded-2xl p-2.5 sm:p-3.5 transition-all">
          {/* Header Bar when Collapsed or Expanded Header */}
          <div className="flex items-center justify-between pb-2 mb-2 border-b border-slate-100 text-xs font-medium">
            ...
            <div className="flex items-center gap-1">
              ...
              <button
                onClick={clear}
                className="p-1 hover:bg-red-50 text-slate-400 hover:text-red-600 rounded-lg transition-colors ms-1"
            ...
            <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-3 pt-0.5">
              ...
              <div className="flex items-center gap-2.5 overflow-x-auto no-scrollbar pt-2.5 pb-1 px-1">
                <AnimatePresence mode="popLayout">
                  {products.map((p) => (
                    <motion.div
                      ...
                      className="group relative flex items-center gap-2 bg-slate-50 border border-slate-200/80 p-1.5 pe-3 rounded-xl shrink-0 hover:border-amber-400 hover:bg-amber-50/40 transition-all cursor-pointer"
                    >
                      <button
                        onClick={(e) => {
                          e.stopPropagation();
                ...
                {products.length < 4 && (
                  <button
                    ...
                    className="flex items-center gap-1.5 h-12 px-3 border border-dashed border-slate-300 hover:border-amber-500 hover:bg-amber-50/50 rounded-xl text-slate-400 hover:text-amber-700 text-xs font-medium transition-all shrink-0"
                  >
              ...
              <div className="flex items-center gap-2 shrink-0 pt-2 sm:pt-0 border-t sm:border-t-0 border-slate-100">
                <button
                  ...
                  className="w-full sm:w-auto flex items-center justify-center gap-2 bg-gradient-to-r from-amber-600 to-amber-700 hover:from-amber-700 hover:to-amber-800 text-white px-5 py-2.5 rounded-xl font-black text-xs shadow-md shadow-amber-600/20 transition-all hover:shadow-lg disabled:opacity-50 disabled:cursor-not-allowed disabled:shadow-none"
                >
