...
export const ComparatorTable: React.FC<ComparatorTableProps> = ({
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-2.5 sm:gap-4">
          {products.map((p, idx) => {
            ...
            return (
              <div
                key={p.id}
                className={`bg-white rounded-2xl sm:rounded-3xl border p-3 sm:p-5 flex flex-col justify-between transition-all shadow-sm ${
                  isPinned
                ...
                <div>
                  ...
                  <div className="flex items-center justify-between mb-2 sm:mb-3">
                    ...
                    <button
                      ...
                      className="w-5 h-5 sm:w-6 sm:h-6 bg-slate-100 hover:bg-red-100 hover:text-red-600 text-slate-400 rounded-full flex items-center justify-center transition-colors"
                    >
                  ...
                  <h3
                    onClick={() => navigate(`/product/${p.id}`)}
                    className="font-bold text-xs sm:text-sm text-slate-900 line-clamp-2 hover:text-amber-600 transition-colors cursor-pointer mb-1 sm:mb-2"
                  >
                    {p.name}
                  </h3>
                  ...
                  {/* Score Bar */}
                  <div className="bg-slate-50 rounded-xl sm:rounded-2xl p-2 sm:p-3 border border-slate-100 mb-3 sm:mb-4">
                    <div className="flex items-center justify-between text-[10px] sm:text-xs font-bold mb-1">
                      <span className="text-slate-500">Score</span>
                      <span className={isWinner ? "text-amber-600 font-black" : "text-slate-800"}>
                        {score}/100
                      </span>
                    </div>
                    <div className="w-full bg-slate-200 h-1.5 sm:h-2 rounded-full overflow-hidden">
                      <div
                        className={`h-full rounded-full ${
                          isWinner ? "bg-gradient-to-r from-amber-500 to-orange-500" : "bg-slate-700"
                        }`}
                        style={{ width: `${score}%` }}
                      />
                    </div>
                  ...
                  {/* Specs List in Card */}
                  <div className="space-y-1 sm:space-y-2 border-t border-slate-100 pt-2 sm:pt-3 text-[10px] sm:text-xs">
                    {specKeys.map((key) => {
                      const val = specs[key] || "-";
                      const isDiff = pinnedSpecs && pinnedSpecs[key] !== val;
                      return (
                        <div key={key} className={`flex justify-between p-1 sm:p-1.5 rounded-lg ${isDiff ? "bg-amber-50/70 font-bold text-amber-900" : "text-slate-700"}`}>
                          <span className="text-slate-400 font-medium capitalize truncate me-1">{key}:</span>
          ...
          {products.length < 4 && (
            <div
              ...
              className="border-2 border-dashed border-slate-300 hover:border-amber-500 hover:bg-amber-50/20 rounded-2xl sm:rounded-3xl p-4 sm:p-8 flex flex-col items-center justify-center gap-2 sm:gap-3 cursor-pointer text-slate-400 hover:text-amber-700 transition-all min-h-[250px] sm:min-h-[350px]"
            >
              <div className="w-10 h-10 sm:w-12 sm:h-12 rounded-2xl bg-slate-100 flex items-center justify-center text-slate-400">
                <Plus className="w-5 h-5 sm:w-6 sm:h-6" />
              </div>
              <span className="font-bold text-xs sm:text-sm text-center">Ajouter un produit</span>
              <span className="text-[10px] sm:text-xs text-slate-400 text-center">Comparez jusqu'à 4 articles</span>
            </div>
          )}
        </div>
      </div>
  ...
  // ----------------------------------------------------
  return (
    <div className="bg-white border border-slate-200/80 rounded-2xl sm:rounded-3xl shadow-lg overflow-hidden">
        ...
        <table className="w-full text-left border-collapse min-w-full sm:min-w-[700px]">
          ...
          <thead className="sticky top-0 z-30 bg-white shadow-sm border-b border-slate-200">
            <tr>
              ...
              <th className="p-2 sm:p-4 bg-slate-100 sm:bg-slate-50 w-20 sm:w-48 md:w-56 min-w-[80px] sm:min-w-[140px] border-r border-slate-200/60 align-bottom sticky left-0 z-40 shadow-[4px_0_8px_rgba(0,0,0,0.08)]">
                <div className="pb-1">
                  <span className="text-[9px] sm:text-xs font-extrabold text-slate-400 uppercase tracking-wider block">
              ...
              {products.map((p, idx) => {
                ...
                return (
                  <th
                    key={p.id}
                    className={`p-2 sm:p-4 relative align-top min-w-[125px] sm:min-w-[180px] max-w-[160px] sm:max-w-none border-r border-slate-100 last:border-r-0 ${
                  ...
                  >
                    ...
                    <div className="flex items-center justify-between mb-1.5 sm:mb-2">
                      ...
                      <button
                        onClick={() => onRemoveProduct(p.id)}
                        className="w-5 h-5 sm:w-6 sm:h-6 bg-slate-100 hover:bg-red-100 hover:text-red-600 text-slate-400 rounded-full flex items-center justify-center transition-colors ms-auto"
                        title="Retirer"
                      >
                        <X className="w-3 h-3 sm:w-3.5 sm:h-3.5" />
                      </button>
                    ...
                    >
                      ...
                      <h3
                        onClick={() => navigate(`/product/${p.id}`)}
                        className="font-bold text-[11px] sm:text-sm text-slate-900 line-clamp-2 hover:text-amber-600 transition-colors cursor-pointer mb-1 min-h-[28px] sm:min-h-[36px]"
                      >
                        {p.name}
                      </h3>
                      ...
                      <div className="w-full bg-slate-100 rounded-lg sm:rounded-xl p-1 sm:p-2">
                        <div className="flex items-center justify-between text-[9px] sm:text-[11px] font-bold mb-0.5 sm:mb-1">
                          <span className="text-slate-500">{t("Score") || "Score"}</span>
                          <span className={isWinner ? "text-amber-600 font-extrabold" : "text-slate-800"}>
                            {score} / 100
                          </span>
                        </div>
          ...
          <tbody className="divide-y divide-slate-100 text-xs">
            ...
            <tr className="bg-slate-100 border-y border-slate-200/80">
              <td colSpan={products.length + 1} className="p-0 bg-slate-100">
                <div className="sticky left-0 z-20 w-max px-2.5 sm:px-4 py-1.5 sm:py-2 font-extrabold text-slate-800 text-[10px] sm:text-[11px] uppercase tracking-wider">
            ...
            {/* Price Comparison */}
            <tr>
              <td className="p-2 sm:p-4 font-semibold text-slate-700 text-[10px] sm:text-xs bg-slate-100 sm:bg-slate-50 border-r border-slate-200/60 sticky left-0 z-20 shadow-[2px_0_5px_rgba(0,0,0,0.04)]">
              ...
              {products.map((p) => {
                ...
                return (
                  <td key={p.id} className="p-2 sm:p-4 text-center border-r border-slate-100">
                    <span className={`font-bold text-xs sm:text-sm ${isMinPrice ? "text-emerald-600" : "text-slate-900"}`}>
                      {formatPrice(p.price)}
                    </span>
                    {isMinPrice && (
            ...
            <tr>
              ...
              {products.map((p) => (
                <td key={p.id} className="p-2 sm:p-4 text-center font-medium text-slate-800 text-[10px] sm:text-xs border-r border-slate-100">
              ...
              {products.map((p) => {
                ...
                return (
                  <td key={p.id} className="p-2 sm:p-4 text-center border-r border-slate-100">
                    {rating !== null ? (
                      <>
                        <div className="inline-flex items-center gap-1 font-bold text-slate-900 bg-amber-50 px-1.5 py-0.5 sm:px-2.5 sm:py-1 rounded-lg border border-amber-200 text-[9px] sm:text-xs">
            ...
            <tr>
              ...
              {products.map((p) => (
                <td key={p.id} className="p-2 sm:p-4 text-center border-r border-slate-100 text-[10px] sm:text-xs font-medium">
                  {p.freeShipping ? (
                    ...
                    <span className="text-slate-600">{t("Standard Express") || "Standard Express"}</span>
                  )}
                </td>
              ))}
            </tr>
            ...
            <tr>
              ...
              {products.map((p) => (
                <td key={p.id} className="p-2 sm:p-4 text-center border-r border-slate-100 text-[10px] sm:text-xs font-medium text-slate-800">
            ...
            {specKeys.map((key) => (
              <tr key={key} className="hover:bg-slate-50/40 transition-colors">
                <td className="p-2 sm:p-4 font-semibold text-slate-700 text-[10px] sm:text-xs bg-slate-100 sm:bg-slate-50 border-r border-slate-200/60 sticky left-0 z-20 shadow-[2px_0_5px_rgba(0,0,0,0.04)] capitalize">
                  {key}
                </td>
                {products.map((p, idx) => {
                  ...
                  return (
                    <td
                      key={p.id}
                      className={`p-2 sm:p-4 text-center border-r border-slate-100 text-[10px] sm:text-xs font-medium ${
                        isDiffFromPinned ? "bg-amber-50/50 text-amber-900 font-bold" : "text-slate-800"
                      }`}
                    >
                      {val}
                    </td>
