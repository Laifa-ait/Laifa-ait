 ...
 * In production: strictly enforces CSRF_SECRET presence (>= 32 chars and non-weak).
 * If absent or weak in production, throws a fatal error to fail-closed.
 * In dev/test: generates an ephemeral cryptographically secure 256-bit secret.
 */
export function getCsrfSecret(): string {
  ...
  if (process.env.NODE_ENV === "production") {
    safeLogger.error(
      "[Olmart Security] ❌ FATAL: CSRF_SECRET environment variable is missing, too short (< 32 chars), or insecure in production."
    );
    throw new Error("CSRF_SECRET is required and must be at least 32 characters in production");
  }

  if (!autoGeneratedSecret) {
    autoGeneratedSecret = crypto.randomBytes(32).toString("hex");
  }
...
/**
 * Validate CSRF secret configuration during startup
 */
