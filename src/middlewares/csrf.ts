 ...
 * Uses CSRF_SECRET if provided (>= 32 chars).
 * If absent, generates an ephemeral cryptographically secure 256-bit secret so the server never crashes on startup.
 */
function getCsrfSecret(): string {
  ...
  if (!autoGeneratedSecret) {
    ...
    if (process.env.NODE_ENV === "production") {
      safeLogger.warn(
        "[Olmart Security] ⚠️ CSRF_SECRET environment variable not set. Using ephemeral cryptographically secure 256-bit secret."
      );
    }
  }
