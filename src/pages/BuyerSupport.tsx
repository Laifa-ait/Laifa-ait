...
export interface SupportTicket {
  ...
  lastMessageAt?: string | Date | { seconds: number; nanoseconds: number };
  resolvedAt?: string | Date | { seconds: number; nanoseconds: number };
...
export const BuyerSupport: React.FC = () => {
  ...
  useEffect(() => {
    ...
    fetchTickets();
    const interval = setInterval(fetchTickets, 8000); // Poll every 8 seconds
    return () => clearInterval(interval);
  }, [currentUser]);

  // 2. Fetch Messages with Polling
  useEffect(() => {
    if (!selectedTicket) {
      setMessages([]);
      return;
    ...
    const fetchMessages = async () => {
      try {
        const data = await apiGet<{ messages: SupportMessage[] }>(`/api/v1/support/tickets/${encodeURIComponent(selectedTicket)}/messages`);
    ...
    fetchMessages();
    const interval = setInterval(fetchMessages, 4000); // Poll messages every 4 seconds for immediate chat feedback
    return () => clearInterval(interval);
  }, [selectedTicket]);
  ...
  const handleCreateTicket = async (e: React.FormEvent) => {
    ...
    try {
      const name = userProfile?.shopName || userProfile?.displayName || 'Client';
      const data = await apiPost<SupportTicket>("/api/v1/support/tickets", {
      ...
      if (data && data.id) {
        ...
        const fresh = await apiGet<{ tickets: SupportTicket[] }>('/api/v1/support/tickets');
  ...
  const handleSendMessage = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Refresh tickets as well to update last message info
      const fresh = await apiGet<{ tickets: SupportTicket[] }>('/api/v1/support/tickets');
      if (fresh && fresh.tickets) {
        setTickets(fresh.tickets);
      }
  ...
  const handleReopenTicket = async () => {
    ...
    try {
      await apiPost(`/api/v1/support/tickets/${encodeURIComponent(selectedTicket)}/reopen`, {});
    ...
    try {
      ...
      reader.onload = async () => {
        try {
          ...
          const msg = error instanceof Error ? error.message : "Le fichier est trop volumineux. La taille maximale autorisée est de 1 Mo.";
          toast.error(msg);
        } finally {
          setUploading(false);
          if (fileInputRef.current) fileInputRef.current.value = '';
                  ...
                  <div className="flex-1 min-w-0 pr-4">
                    ...
                    <div className="flex items-center gap-2 mt-2">
                       ...
                       <span className="text-[10px] text-zinc-400 uppercase font-bold tracking-widest rtl:tracking-normal">
                          {ticket.createdAt ? new Date(normalizeTimestamp(ticket.createdAt).toMillis()).toLocaleDateString('fr-FR') : ''}
                       </span>
                    </div>
                  </div>
                        ...
                        <div className={`max-w-[75%] rounded-2xl p-5 ${isSeller ? 'bg-white rounded-tr-sm border border-zinc-100 shadow-sm' : 'bg-zinc-50 rounded-tl-sm border border-zinc-200'}`}>
                          ...
                          {m.createdAt && (
                            <div className="mt-3 flex items-center justify-end gap-1.5 text-[9px] font-sans font-bold uppercase text-zinc-300">
                              ...
                              {m.createdAt ? new Date(normalizeTimestamp(m.createdAt).toMillis()).toLocaleString('fr-FR') : ''}
                            </div>
                          )}
                        </div>
                      </div>
                ...
                <div className="p-4 md:p-6 bg-white border-t border-zinc-100 shrink-0">
                 ...
                 <div className="p-8 bg-zinc-50 border-t border-zinc-100 text-center space-y-4 shrink-0">
                    ...
                    {(() => {
                       ...
                       let eligible = true;
                       if (ts) {
                          const resDate = new Date(normalizeTimestamp(ts).toMillis());
          ...
          <div className="fixed inset-0 bg-zinc-950/40 backdrop-blur-sm z-50 flex items-center justify-center p-4">
            ...
            >
              ...
              <form onSubmit={handleCreateTicket} className="p-8 space-y-6">
                <div>
                  <label className="block text-xs font-sans font-bold text-zinc-400 uppercase tracking-widest rtl:tracking-normal mb-2">{t("Sujet du ticket")}</label>
                  <input 
                    type="text" 
                    required
