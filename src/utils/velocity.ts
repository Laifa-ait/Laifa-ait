...
export function enqueueSellerVelocityCheck(sellerId: string): void {
  // 1. Write job to durable Firestore queue asynchronously (persisted across process restarts/SIGTERM)
  ...
  // 2. Immediate non-blocking in-memory execution
  const checkPromise = (async () => {
    await processVelocityJob(sellerId);
  })();
