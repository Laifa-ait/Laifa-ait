...
export async function checkSellerVelocityLimit(sellerId: string, transaction?: firestore.Transaction): Promise<void> {
  try {
      ...
      const promise = (async (): Promise<boolean> => {
        try {
          ...
          if (buyerData && buyerData.createdAt) {
            ...
            isOldAccount = (Date.now() - createdAtTime) > 2 * 24 * 60 * 60 * 1000;
          }
