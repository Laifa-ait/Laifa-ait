...
router.get("/homepage/sections", async (req: Request, res: Response) => {
  try {
    const snap = await db.collection("homepageSections").orderBy("order", "asc").get();
    const sections = snap.docs.map(doc => ({ id: doc.id, ...doc.data() }));
...
router.post("/homepage/sections", authenticateToken, authorizeAdmin, async (req: AuthenticatedRequest, res: Response) => {
  try {
    const sectionData = { ...req.body, createdAt: admin.firestore.FieldValue.serverTimestamp() };
    const ref = await db.collection("homepageSections").add(sectionData);
    res.json({ success: true, section: { id: ref.id, ...sectionData } });
  } catch (err: unknown) {
    res.status(500).json({ error: err instanceof Error ? err.message : "Erreur interne" });
  }
});

router.put("/homepage/sections/:id", authenticateToken, authorizeAdmin, async (req: AuthenticatedRequest, res: Response) => {
  try {
    const { id } = req.params;
    await db.collection("homepageSections").doc(id).update({
      ...req.body,
      updatedAt: admin.firestore.FieldValue.serverTimestamp(),
    });
    res.json({ success: true });
...
router.delete("/homepage/sections/:id", authenticateToken, authorizeAdmin, async (req: AuthenticatedRequest, res: Response) => {
  try {
    const { id } = req.params;
    await db.collection("homepageSections").doc(id).delete();
    res.json({ success: true });
  } catch (err: unknown) {
    res.status(500).json({ error: err instanceof Error ? err.message : "Erreur interne" });
  }
...
// Homepage Banners (Specific slot management)
router.get("/homepage/banners", async (req: Request, res: Response) => {
  try {
    const snap = await db.collection("homepageBanners").get();
    const banners = snap.docs.map(doc => ({ id: doc.id, ...doc.data() }));
    res.json({ success: true, banners });
  } catch (err: unknown) {
    res.status(500).json({ error: err instanceof Error ? err.message : "Erreur interne" });
...
router.put("/homepage/banners/:id", authenticateToken, authorizeAdmin, async (req: AuthenticatedRequest, res: Response) => {
  try {
    const { id } = req.params;
    await db.collection("homepageBanners").doc(id).set(
      {
        ...req.body,
        updatedAt: admin.firestore.FieldValue.serverTimestamp(),
      },
...
router.delete("/homepage/banners/:id", authenticateToken, authorizeAdmin, async (req: AuthenticatedRequest, res: Response) => {
  try {
    const { id } = req.params;
    await db.collection("homepageBanners").doc(id).delete();
    res.json({ success: true });
  } catch (err: unknown) {
    res.status(500).json({ error: err instanceof Error ? err.message : "Erreur interne" });
  }
